...
def predict_single(
    ...
    if len(image_bytes) > settings.max_upload_size:
        raise HTTPException(
            status_code=400, detail="Image exceeds maximum allowed size"
        )

    if len(image_bytes) == 0:
        raise HTTPException(status_code=400, detail="Image data is empty or invalid")

    # Demo prediction fallback because real model file is not available yet
    result = {
        "primary_label": "melanocytic_nevus",
        "confidence": 0.87,
        "risk_level": "Low",
        "predictions": [
            {"label": "melanocytic_nevus", "confidence": 0.87},
            {"label": "benign_keratosis", "confidence": 0.09},
            {"label": "dermatofibroma", "confidence": 0.04},
        ],
        "uncertain": False,
    }
